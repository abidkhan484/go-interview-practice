...
func MinCoins(amount int, denominations []int) int {
	...
	for i := numDenominations - 1; i >= 0; i-- {
		if amount == 0 {
			break
		}
	...
	if amount > 0 {
		return -1 // Cannot make the amount with given denominations
	}
	return minCoins
}
...
func CoinCombination(amount int, denominations []int) map[int]int {
	...
	for i := numDenominations - 1; i >= 0; i-- {
		...
		if coin <= amount {
			numCoins := amount / coin
			coinMap[coin] = numCoins
			amount -= numCoins * coin
		}
	}
	if amount > 0 {
		return map[int]int{} // Cannot make the amount with given denominations
	}
