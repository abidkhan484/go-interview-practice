...
func OptimizedSort(data []int) []int {
	// using standart sort (uses quicksort)
	result := make([]int, len(data))
	copy(result, data)
	sort.Ints(result)
...
func OptimizedCalculation(n int) int {
	...
	// Sum is F(n+2) - F(1) which is F(n+2) - 1
	return current - 1
}
...
func OptimizedSearch(text, substr string) map[int]string {
	// Edge case: empty substring causes infinite loop or invalid matches
	if len(substr) == 0 || len(text) < len(substr) {
		return make(map[int]string)
	}

	// Pre-allocate map with a small baseline capacity to reduce resizing overhead
	result := make(map[int]string, 4)
	subLen := len(substr)
	maxIdx := len(text) - subLen

	for i := 0; i <= maxIdx; i++ {
		// Slice the window from the original text (creates a string header, zero allocations)
		window := text[i : i+subLen]

		// EqualFold performs a case-insensitive comparison without allocating new strings
