...
func (e *InsufficientFundsError) Error() string {
	return fmt.Sprintf("insufficient founds error: %f", e.amuont)
}
...
func (e *NegativeAmountError) Error() string {
	return fmt.Sprintf("negative amount error: %f", e.amount)
}
...
func (e *ExceedsLimitError) Error() string {
	return fmt.Sprintf("exeeds limit error: %f", e.amount)
}
...
func NewBankAccount(id, owner string, initialBalance, minBalance float64) (*BankAccount, error) {
	if id == "" || owner == "" {
		return nil, &AccountError{message: "Invalide ID or Owner"}
	}
	...
	if initialBalance < minBalance {
		return nil, &InsufficientFundsError{amuont: initialBalance}
	}
...
func (a *BankAccount) Withdraw(amount float64) error {
	...
	if a.Balance-amount < a.MinBalance {
		return &InsufficientFundsError{amuont: amount}
	}
